locals {
  deployment = {
    nodered = {
      container_ct = length(var.ext_port["nodered"][terraform.workspace])
    ...
    influxdb = {
      container_ct = length(var.ext_port["influxdb"][terraform.workspace])
    ...
    grafana = {
      container_ct = length(var.ext_port["grafana"][terraform.workspace])
